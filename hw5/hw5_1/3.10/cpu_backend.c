...
static inline Result iterate_secant_xy(double x0, double y0, int max_iter, double tol, double eps) {
    Result res;
    res.converged = 0;
    res.root_id = -1;
    res.iter = max_iter;
    ...
    /* Purely real perturbation keeps the 3 main RGB sectors symmetric. */
    double x_prev = x0;
    double y_prev = y0;
    double x_cur  = x0 + eps;
    double y_cur  = y0;
    ...
    for (int k = 0; k < max_iter; ++k) {
        ...
        if (denom < 1e-28) {
            return classify_secant_fallback(x0, y0, max_iter, tol);
        }
...
static inline void shade_secant_rgb(unsigned char *r,
                                    ...
                                    int iter,
                                    int max_iter) {
    /* RGB root coloring like the teacher's first image:
       root 0 -> red, root 1 -> green, root 2 -> blue.
       Increase whitening near slow-converging boundaries. */
    double br, bg, bb;
    if (root_id == 0) {
        br = 255.0; bg = 40.0;  bb = 40.0;
    } else if (root_id == 1) {
        br = 40.0;  bg = 220.0; bb = 40.0;
    } else {
        br = 40.0;  bg = 60.0;  bb = 255.0;
    }
    ...
    /* Slow convergence => lighter/whiter edge. */
    double whiten = 0.65 * pow(t, 0.85);
...
void render_cpu(unsigned char *img, const FractalConfig *cfg) {
    ...
    /* Important: for Newton two-color rendering, conjugate rows look identical,
       so we can mirror the top half to the bottom half. For secant RGB rendering,
       conjugation swaps the two complex roots (green <-> blue), so blindly copying
       rows makes the upper and lower halves incorrectly have the same color. */
    const int mirror_rows = (cfg->method == METHOD_NEWTON) && has_real_axis_symmetry(cfg);
    ...
    for (int j = 0; j < rows_to_compute; ++j) {
        ...
        if (cfg->method == METHOD_SECANT) {
            render_row_secant_scalar(row_top, xcoords, y, width, cfg->max_iter, cfg->tol, secant_eps);
        } else {
#ifdef __AVX__
