...
static inline Result iterate_secant_xy(double x0, double y0,
    ...
    const double switch_tol2 = 256.0 * tol2;  /* 比之前更早切 Newton */
...
typedef struct {
    unsigned char rgb[3][3]; /* [root_id][channel] */
} SecantLUTEntry;

static SecantLUTEntry *build_secant_lut(int max_iter) {
    SecantLUTEntry *lut = (SecantLUTEntry *)malloc((size_t)(max_iter + 1) * sizeof(SecantLUTEntry));
    if (!lut) return NULL;
    ...
    for (int iter = 0; iter <= max_iter; ++iter) {
        ...
        const double base[3][3] = {
            {255.0,  50.0,  50.0},  /* root 0 -> red */
            { 50.0, 225.0,  50.0},  /* root 1 -> green */
            { 50.0,  70.0, 255.0}   /* root 2 -> blue */
        };

        for (int root = 0; root < 3; ++root) {
            double r = (base[root][0] * (1.0 - whiten) + 255.0 * whiten) * brightness;
            double g = (base[root][1] * (1.0 - whiten) + 255.0 * whiten) * brightness;
            double b = (base[root][2] * (1.0 - whiten) + 255.0 * whiten) * brightness;
            lut[iter].rgb[root][0] = clamp_u8(r);
            lut[iter].rgb[root][1] = clamp_u8(g);
            lut[iter].rgb[root][2] = clamp_u8(b);
        }
    }
...
static inline void shade_secant_rgb_lut(unsigned char *dst,
                                        int root_id,
                                        int iter,
                                        int max_iter,
                                        const SecantLUTEntry *lut) {
    if (iter < 0) iter = 0;
    if (iter > max_iter) iter = max_iter;
    if (root_id < 0) root_id = 0;
    if (root_id > 2) root_id = 2;

    dst[0] = lut[iter].rgb[root_id][0];
    dst[1] = lut[iter].rgb[root_id][1];
    dst[2] = lut[iter].rgb[root_id][2];
}
...
static inline void render_row_newton_scalar(unsigned char *row,
    ...
    for (int i = 0; i < width; ++i) {
        ...
        shade_newton_two_color(&row[(size_t)3*i + 0],
                               &row[(size_t)3*i + 1],
                               &row[(size_t)3*i + 2],
...
static inline void render_row_secant_scalar(unsigned char *row,
                                            unsigned char *row_root,
                                            unsigned short *row_iter,
                                            const double *xcoords,
                                            double y,
                                            int width,
                                            int max_iter,
                                            ...
                                            const SecantLUTEntry *lut) {
    for (int i = 0; i < width; ++i) {
        Result res = iterate_secant_xy(xcoords[i], y, max_iter, tol, eps);

        row_root[i] = (unsigned char)res.root_id;
        row_iter[i] = (unsigned short)res.iter;

        shade_secant_rgb_lut(row + (size_t)3 * (size_t)i,
                             res.root_id, res.iter, max_iter, lut);
    }
}
...
static inline int movemask_lt(__m256d a, __m256d b) {
...
static inline void render_row_newton_simd(unsigned char *row,
    ...
    for (; i + 4 <= width; i += 4) {
        ...
        for (int k = 0; k < max_iter && active_mask; ++k) {
            ...
            int singular_mask = movemask_lt(r2, singular_r2v) & active_mask;
            ...
            int done_mask = movemask_lt(step2, tol2v) & active_mask & (~singular_mask);
        ...
        for (int lane = 0; lane < 4; ++lane) {
            int root = converged[lane] ? nearest_root_xy(xf[lane], yf[lane]) : -1;
            shade_newton_two_color(&row[(size_t)3*(i+lane) + 0],
                                   &row[(size_t)3*(i+lane) + 1],
                                   &row[(size_t)3*(i+lane) + 2],
                                   root, iters[lane], max_iter, converged[lane]);
        }
    }
...
void render_cpu(unsigned char *img, const FractalConfig *cfg) {
    ...
    SecantLUTEntry *secant_lut = NULL;
    if (cfg->method == METHOD_SECANT) {
        secant_lut = build_secant_lut(cfg->max_iter);
        if (!secant_lut) {
            fprintf(stderr, "[cpu] failed to allocate secant LUT\n");
            free(xcoords);
            return;
        }
    }
    ...
    fprintf(stderr, "[cpu] Secant SIMD (AVX) = off\n");
    fprintf(stderr, "[cpu] Secant LUT shading = on\n");

    #pragma omp parallel
#endif
    {
        /* 每个线程只分配一次 */
        unsigned char *row_root = NULL;
        unsigned short *row_iter = NULL;

        if (cfg->method == METHOD_SECANT) {
            row_root = (unsigned char *)malloc((size_t)width * sizeof(unsigned char));
            row_iter = (unsigned short *)malloc((size_t)width * sizeof(unsigned short));
            if (!row_root || !row_iter) {
                fprintf(stderr, "[cpu] failed to allocate per-thread secant buffers\n");
                free(row_root);
                free(row_iter);
                row_root = NULL;
                row_iter = NULL;
        ...
        for (int j = 0; j < rows_to_compute; ++j) {
            ...
            if (cfg->method == METHOD_NEWTON) {
                ...
                if (!row_root || !row_iter) continue;

                render_row_secant_scalar(row_top, row_root, row_iter, xcoords, y, width,
                                         cfg->max_iter, cfg->tol, secant_eps, secant_lut);

                if (secant_conj_mirror) {
                    int mj = height - 1 - j;
                    if (mj != j) {
                        unsigned char *row_bottom = img + (size_t)3 * (size_t)mj * (size_t)width;
                        for (int i = 0; i < width; ++i) {
                            int bot_root = conjugate_swap_root((int)row_root[i]);
                            int bot_iter = (int)row_iter[i];
                            shade_secant_rgb_lut(row_bottom + (size_t)3 * (size_t)i,
                                                 bot_root, bot_iter, cfg->max_iter, secant_lut);
                        }
                    }
                }
            }
