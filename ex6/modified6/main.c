...
int main() {
    ...
    FILE *fp_actual, *fp_all_pred, *fp_best_pred, *fp_gp1, *fp_gp2;
    FILE *fp_fit_curve, *fp_fit_pred, *fp_scheme_bar;
    ...
    /* 柱状图数据：只保留 [-1100, 2000] 内的预测值 */
    fp_scheme_bar = fopen("scheme_compare.dat", "w");
    if (fp_scheme_bar == NULL) {
        printf("Cannot open scheme_compare.dat\n");
        return 1;
    }

    for (i = 0; i < result_count; i++) {
        if (results[i].pred2026 >= -1100.0 && results[i].pred2026 <= 2000.0) {
            fprintf(fp_scheme_bar, "\"%d\" %.6f\n",
                    results[i].start_year,
                    results[i].pred2026);
        }
    }

    if (quad_pred2026 >= -1100.0 && quad_pred2026 <= 2000.0) {
        fprintf(fp_scheme_bar, "\"fit\" %.6f\n", quad_pred2026);
    }
    fclose(fp_scheme_bar);
    ...
    fprintf(fp_gp1, "set terminal pngcairo size 1500,900 enhanced font 'Arial,14'\n");
    ...
    /* ---------- 图2：预测方案对比柱状图 ---------- */
    fp_gp2 = fopen("plot_bar.gnuplot", "w");
    if (fp_gp2 == NULL) {
        printf("Cannot open plot_bar.gnuplot\n");
        return 1;
    }

    fprintf(fp_gp2, "set terminal pngcairo size 1400,800 enhanced font 'Arial,14'\n");
    fprintf(fp_gp2, "set output 'scheme_comparison.png'\n");
    ...
    fprintf(fp_gp2, "set ylabel 'Predicted value for 2026'\n");
    fprintf(fp_gp2, "set grid ytics\n");
    fprintf(fp_gp2, "set yrange [-1100:2000]\n");
    fprintf(fp_gp2, "set style data histograms\n");
    fprintf(fp_gp2, "set style fill solid 0.8 border -1\n");
    fprintf(fp_gp2, "set boxwidth 0.7\n");
    fprintf(fp_gp2, "set xtics rotate by -45\n");
    fprintf(fp_gp2, "plot 'scheme_compare.dat' using 2:xtic(1) title 'Prediction'\n");
    ...
    printf("7. scheme_compare.dat\n");
    printf("8. plot_main.gnuplot\n");
    printf("9. plot_bar.gnuplot\n");
    printf("10. interpolation_extrapolation.png\n");
    printf("11. scheme_comparison.png\n");

    system("gnuplot plot_main.gnuplot");
    system("gnuplot plot_bar.gnuplot");
