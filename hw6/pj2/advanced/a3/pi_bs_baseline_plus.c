...
static void bs_parallel(mpz_t P, mpz_t Q, mpz_t T, long a, long b, int depth, int max_depth){
    ...
    if (depth < g_merge_task_depth && n >= g_merge_task_min_size){
...
int main(int argc, char **argv){
...
#endif
    if (max_depth == 0){
        max_depth = auto_task_depth(max_threads);
    }

    long terms = (long)(digits / 14.181647462725477) + 10;
    mpfr_prec_t prec = (mpfr_prec_t)(digits * 3.32192809488736234787) + 64;

    tls_lhs = malloc((size_t)max_threads * sizeof(mpz_t));
    tls_rhs = malloc((size_t)max_threads * sizeof(mpz_t));
    #pragma omp parallel
    {
    ...
    printf("method=chudnovsky_baseline_plus\n");
    printf("digits=%ld\n", digits);
    printf("terms=%ld\n", terms);
    printf("task_depth=%d\n", max_depth);
    printf("leaf_size=%ld\n", g_leaf_size);
